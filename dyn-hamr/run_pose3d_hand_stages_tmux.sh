...
SEQ="${SEQ:-segment_000_ch2_undistort}"
STAGE="${STAGE:-all}"
RUN_ID="${RUN_ID:-$(date +%Y-%m-%d_%H-%M-%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-../outputs/pose3d_hand_stages}"
RUN_DIR="${RUN_DIR:-${OUTPUT_ROOT}/${SEQ}_${RUN_ID}}"

POSE3D_HAND="${POSE3D_HAND:-../demo/${SEQ}.pose3d_hand}"
TRACK_INFO="${TRACK_INFO:-../demo/${SEQ}_track_info.npy}"
KEYPOINTS_NPY="${KEYPOINTS_NPY:-${RUN_DIR}/${SEQ}_keypoints.npy}"
IMAGE_ROOT="${IMAGE_ROOT:-${RUN_DIR}/${SEQ}_frames}"
...
DEVICE_OVERRIDE="${DEVICE_OVERRIDE:-cuda:3}"
WORK_DIR="${WORK_DIR:-${RUN_DIR}}"
OUTPUT="${OUTPUT:-${RUN_DIR}/${SEQ}_export.pose3d_hand}"
LOG_FILE="${LOG_FILE:-${WORK_DIR}/run_${STAGE}_${RUN_ID}.log}"
